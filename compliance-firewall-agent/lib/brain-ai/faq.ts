...
const FAQ_DB: FaqEntry[] = [
  // ── ASSISTANT IDENTITY (must work without an LLM key) ───────────────
  {
    keywords: ["who are you", "what are you", "who is brain", "your name", "what is brain ai", "introduce yourself", "who made you", "about you", "tell me about yourself", "hermes"],
    answer:
      "I'm **Brain AI** — the compliance assistant built into **HoundShield**, the local-only AI compliance firewall.\n\nI run on the HERMES agent architecture and I'm a senior expert in:\n- **CMMC Level 2** & all 110 NIST 800-171 Rev 2 controls\n- **SPRS scoring** (−203 to +110)\n- **HIPAA PHI** (all 18 Safe Harbor identifiers) & **SOC 2**\n- **CUI detection** — 16 engines, under 10ms per prompt\n\nAsk me anything: \"How does CMMC Level 2 work?\", \"What can you detect?\", or \"How do I install HoundShield?\"",
  },
  {
    keywords: ["hello", "hi there", "hey brain", "hey there", "good morning", "good afternoon", "good evening", "howdy", "greetings"],
    answer:
  ...
  // ── SOC 2 ────────────────────────────────────────────────────────────
  {
    ...
    answer:
      "For **SOC 2**, HoundShield covers the Security and Confidentiality trust service criteria around AI data handling. It prevents unauthorized disclosure of customer data through AI tools, maintains an immutable audit trail with SHA-256 hash chain, and provides exportable reports for your SOC 2 auditor.",
  },
  ...
  // ── MCP / DEVELOPER INTEGRATIONS ────────────────────────────────────
  {
    ...
    answer:
      "HoundShield exposes an **MCP Server endpoint** that integrates with Claude Desktop, Cursor, and any MCP-compatible client. Set the HoundShield gateway as your MCP proxy and every tool call passes through compliance scanning automatically. This is the highest-value enterprise integration — contact us for early access at info@houndshield.com.",
  },
  {
    keywords: ["api", "rest api", "api key", "programmatic", "developer", "v1", "api access"],
    answer:
      "**HoundShield REST API v1** (`/api/v1/*`) lets MSPs and developers:\n- Query compliance events programmatically\n- Trigger scans via HTTP\n- Manage team API keys\n- Pull audit data into custom dashboards\n\nAPI access is available on Pro+ plans. Authentication: Bearer token in `x-api-key` header. Full OpenAPI spec coming Q2 2026.",
  },
  {
    keywords: ["managed agents", "anthropic", "claude managed", "hosted agents", "cloud agents"],
    answer:
      "HoundShield supports **Anthropic Managed Agents** integration — composable cloud-hosted agent APIs that connect to the HoundShield compliance gateway. When Claude agents make tool calls or fetch external data, HoundShield intercepts and scans each request in real-time, providing compliance coverage for autonomous AI workflows, not just chat.",
  },
  {
  ...
  // ── MODEL ACCESS ─────────────────────────────────────────────────────
  {
    ...
    answer:
      "HoundShield provides access to **800+ models via OpenRouter** through a single compliant endpoint:\n- OpenAI: GPT-4o, o3, o1\n- Anthropic: Claude 4.6 Sonnet, Claude 3.5 Haiku\n- Google: Gemini 2.5 Pro/Flash\n- Meta: Llama 3.3 70B\n- Qwen: Qwen3 (including free tiers)\n- Mistral: Large, Codestral\n- 800+ more\n\nAll models, all compliant. Use the model slug as the `model` parameter — HoundShield routes to the right provider automatically.",
  },
