...
export function buildComplianceDoc(data: ReportData): jsPDF {
  ...
  if (isSnapshot) {
    ...
    doc.setTextColor(DARK_GREY);
    doc.text(
      "Generated locally in your browser from a single paste. This is NOT the tamper-evident, assessor-reviewable 14-day",
      MARGIN + 5,
      y + 10.5
    );
    doc.text(
    ...
    doc.setFont("helvetica", "bold");
    doc.text("  ILLUSTRATIVE SAMPLE — representative data shown. Your $499 report is generated from your own environment.", MARGIN + 5, y + 6.5);
    doc.setFont("helvetica", "normal");
  }
  ...
  if (blockEvents.length === 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [["Date", "Action", "Risk", "Pattern Detected", "NIST Control", "Control Name", "SPRS Impact", "DCSA"]],
      body: evidenceRows,
      margin: { left: MARGIN, right: MARGIN },
  ...
  const evidenceNote = isSnapshot
    ? "This preview maps findings to NIST 800-171 controls for illustration only. It is not audit evidence " +
      ...
      "environment — is the signed artifact an assessor may review."
  ...
  const attestation = isSnapshot
    ? [
        ...
        "tamper-evident, and it is not a CMMC or assessor-review evidence artifact. Do not submit it to an assessor.",
        "",
