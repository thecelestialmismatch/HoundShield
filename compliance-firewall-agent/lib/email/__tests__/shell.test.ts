...
describe("emailHeader — the brand mark", () => {
  ...
  it("serves the logo from a file that actually ships", () => {
    ...
    expect(path).toBe("/logo-mark-white.png");
    expect(readdirSync(join(APP_ROOT, "public"))).toContain("logo-mark-white.png");
  });

  it("uses the WHITE mark, because the band is dark", () => {
    // public/logo.png is a near-black shield. Pointing the dark band at it is
    // the original defect: an invisible logo that passes every other assertion
    // here. The white recolour is generated by scripts/gen-white-mark.py.
    expect(LOGO_URL).toContain("logo-mark-white");
    expect(LOGO_URL).not.toMatch(/\/logo\.png$/);
  });
  ...
  it("keeps the dark band and a mark that contrasts against it", () => {
    // The pairing is what matters, not either value alone: a dark band with the
    // near-black mark is the invisible-logo defect, and it passed every other
    // assertion in this file while shipping.
    const html = emailHeader();
    expect(html).toContain("background:#0f172a");
    expect(html).toContain("logo-mark-white");
    expect(html).toContain("color:#ffffff"); // the wordmark, legible on navy
  });
