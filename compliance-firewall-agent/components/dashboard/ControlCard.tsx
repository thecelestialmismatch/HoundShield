...
export default function ControlCard({
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 dark:border-white/10 dark:border-slate-700/50 rounded-2xl overflow-hidden">
      {/* ── Header ── */}
      <div className="p-6 pb-4">
        ...
        <p className="text-slate-300 dark:text-slate-300 text-sm leading-relaxed">{control.assessmentQuestion}</p>
      </div>
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 py-3 px-4 rounded-xl border text-sm font-medium transition-all ${
                  isActive
                    ...
                    : "bg-white/[0.05] border-slate-600 text-slate-400 hover:border-slate-500"
                }`}
              >
                <Icon size={16} />
                {config.label}
      ...
      <div className="px-6 pb-4 space-y-3">
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full bg-white/[0.05] border border-slate-600 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-all resize-none"
          />
        </div>
        <button
          ...
          className={`flex items-center gap-2 text-sm py-2 px-3 rounded-lg transition-all ${
            evidenceUploaded
              ...
              : "bg-white/[0.05] text-slate-400 hover:text-slate-500"
          }`}
        >
          <FileCheck size={14} />
      ...
      {/* ── Expandable Details ── */}
      <div className="border-t border-white/10 dark:border-white/10 dark:border-slate-700/50">
        <button
          onClick={() => setShowDetails(!showDetails)}
          className="w-full flex items-center justify-between px-6 py-3 text-sm text-slate-400 hover:text-slate-300 dark:text-slate-300 transition-colors"
        >
            ...
            >
              <div className="px-6 pb-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-slate-300 dark:text-slate-300 text-sm">{control.officialDescription}</p>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {control.affordableTools.map((tool, i) => (
                        <span
                          key={i}
                          className="px-2.5 py-1 bg-brand-500/100/10 border border-brand-500/20 rounded-lg text-xs text-brand-300"
                        >
                          {tool}
                        </span>
                      ))}
                ...
                {/* Metadata row */}
                <div className="flex items-center gap-4 pt-2 border-t border-white/10 dark:border-slate-700/30 text-xs text-slate-500">
