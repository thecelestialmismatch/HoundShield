...
/**
 * The 110-control CMMC self-assessment board. Extracted from the standalone
 * `/command-center/shield/assessment` route so it can render in TWO places from
 * ONE source of truth:
 *   1. its own full-screen route (embedded=false, default), and
 *   2. inline on the after-login `/console` dashboard (embedded=true), so a user
 *      begins their assessment on the same page they land on — no bounce to a
 *      separate deep link.
 *
 * `embedded` only affects the outer chrome (drops the full-viewport min-height so
 * the board nests inside a card). All assessment logic — localStorage
 * persistence, SPRS scoring, filtering — is identical in both mounts.
 */
export interface AssessmentBoardProps {
  /** When true, render for inline mounting (no forced min-h-screen). */
  embedded?: boolean;
}

export default function AssessmentBoard({ embedded = false }: AssessmentBoardProps) {
