 ...
 * Every request passes through HoundShield compliance scanning before
 * reaching the local Ollama instance.
 *
 * Body: Ollama /api/chat format (messages, model, stream, options)
 * Response: HoundShield SSE format (data: { content }..., data: [DONE])
 *
  ...
  try {
        ...
        return NextResponse.json(
          {
            ...
            message:
              `HoundShield blocked this prompt: contains ${scan.classifications.join(", ")} ` +
          ...
          {
            status: 451,
            headers: { "X-HoundShield-Action": "BLOCKED" },
          }
        );
      }
    }
    ...
    if (!result.body) {
      return NextResponse.json(
        ...
        { status: 503, headers: { "X-HoundShield-Ollama": "offline" } }
      );
    }
    ...
    return new Response(proxyOllamaStream(result.body), {
      headers: {
        ...
        "X-HoundShield-Action": "ALLOWED",
        "X-HoundShield-Mode": "air-gap",
      },
    });
  } catch (err) {
