...
/**
 * Account security lived at `/console/security` until the 2026-07-29 dashboard
 * merge; `/console/security` now permanently redirects here.
 *
 * The session check below is deliberately redundant with the fail-closed gate
 * in `app/command-center/layout.tsx` — this page reads identity (`user.email`,
 * 2FA state), so it verifies its own caller rather than trusting an ancestor.
 */
export default async function AccountSecurityPage() {
  const user = await getSessionUser()
  if (!user) redirect('/login?redirect=%2Fcommand-center%2Faccount-security')
