/**
 * HoundShield Streaming Gateway — Shared Provider Types
 *
 ...
 * - `StreamRequest` carries the user-supplied API key so HoundShield never stores provider credentials.
...
/**
 * Inbound streaming request from a HoundShield client.
 *
 * The `api_key` is the end-user's own provider key — HoundShield acts as a
...
export interface StreamRequest {
  ...
  max_tokens?: number;
  /** Must be true — HoundShield only supports streaming mode. */
  stream: true;
  /** User's API key for the target provider (never stored). */
  api_key: string;
  /** HoundShield user identifier for audit logging. */
  user_id: string;
