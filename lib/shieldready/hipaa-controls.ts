/**
 * HIPAA Security Rule Control Mapping — HoundShield
 *
...
export const HIPAA_CONTROLS: HIPAAControl[] = [
  ...
  },
  {
    ...
    aiRiskRelevance:
      "Evaluations must include AI tool usage patterns, PHI exposure incidents via AI, and effectiveness of AI security controls like HoundShield.",
  },
  ...
  },
  {
    ...
    aiRiskRelevance:
      "Every AI interaction involving PHI must be logged. HoundShield provides this via its compliance event audit trail with SHA-256 hash chain integrity.",
  },
  {
    id: "HIPAA-164.312-c1",
    title: "Integrity",
    ...
    aiRiskRelevance:
      "AI-generated clinical content must be validated before use. AI tools must not modify source PHI. HoundShield blockchain anchoring provides tamper-proof evidence.",
  },
  {
    id: "HIPAA-164.312-d",
  ...
  },
  {
    ...
    aiRiskRelevance:
      "PHI sent to AI APIs must use TLS 1.2+. HoundShield intercepts AI traffic before it leaves the network, blocking PHI before it reaches external AI services.",
  },
];
...
function generateRecommendations(
  ...
  if (techGaps.length > 2) {
    recs.push(
      "Technical safeguards need attention — deploy HoundShield AI firewall for real-time PHI interception and audit trail compliance."
    );
  }

  if (score < 60) {
    recs.push(
      "Overall compliance is below 60% — consider engaging a HIPAA compliance consultant alongside HoundShield automated monitoring."
    );
  }

  if (gaps.some((g) => g.controlId === "HIPAA-164.312-b")) {
    recs.push(
      "Audit Controls (§164.312(b)) gap detected — HoundShield automatically logs all AI interactions with PHI for audit compliance."
    );
  }

  if (gaps.some((g) => g.controlId === "HIPAA-164.312-e1")) {
    recs.push(
      "Transmission Security gap — HoundShield blocks PHI before it reaches external AI services, satisfying §164.312(e)(1)."
    );
  }
